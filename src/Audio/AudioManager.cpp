...
std::unordered_map<std::string, ALuint> AudioManager::sLoadedAudio_;
...
void AudioManager::freeLoadedAudios() {
    for (const auto& pair : sLoadedAudio_) {
        ALuint bufferId = pair.second;
        alDeleteBuffers(1, &bufferId);
    }
}

ALuint AudioManager::loadAudioFile(const char* filename) {
    ALenum err, format;
    ALuint buffer;
    SNDFILE* sndfile;
    SF_INFO sfinfo;
...
void AudioManager::playSound(std::string soundName) {
    auto it = sLoadedAudio_.find(soundName);
    if (it != sLoadedAudio_.end()) {
        mSoundSource_->play(it->second);
    }
}
